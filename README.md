# ☁️ Salesforce 길잡이 — 입문자 가이드 챗봇

Salesforce 생태계에 **처음 진입하는 입문자**(어드민 · 개발자 · 컨설턴트 · 슈퍼유저)를 위한
AI 챗봇입니다. **Groq API**(무료)로 답변을 생성하고, HTML 프론트엔드로 동작하며,
**Vercel**로 배포합니다.

- 🎨 Salesforce 느낌의 UI (Salesforce 블루 `#00A1E0`)
- 💬 우측 하단의 작은 채팅 버블 → 클릭하면 부드럽게 펼쳐지는 위젯
- 🇰🇷 한국어 중심 (질문이 영어면 영어로 응답)
- 🔐 API 키는 **서버리스 함수**가 보관 — 브라우저에 절대 노출되지 않음

---

## 📁 프로젝트 구조

```
salesforcechatbot/
├── public/
│   ├── index.html      # 데모 랜딩 페이지 (widget.js를 불러옴)
│   └── widget.js       # 임베드형 챗봇 위젯 (script 한 줄로 어디든 삽입)
├── api/
│   └── chat.js         # Vercel 서버리스 함수: Groq API 프록시 (CORS 허용)
├── vercel.json         # 라우팅 설정
├── package.json        # 메타데이터 (외부 의존성 없음)
├── .env.example        # 환경변수 예시
└── README.md
```

---

## 🔑 필요한 키

**Groq API 키** 1개가 필요합니다. **무료**이며 카드 등록이 필요 없습니다.

1. https://console.groq.com 접속 → 로그인
2. **API Keys** 메뉴에서 **Create API Key** → 키 복사
3. 이 키를 코드에 넣지 말고, 아래 Vercel 환경변수(`GROQ_API_KEY`)로 등록합니다.

> 참고: Groq 무료 티어에는 분당/일일 요청 수 제한(rate limit)이 있지만, 개인·입문용으로는
> 충분합니다.

---

## 🚀 Vercel 배포 방법

1. 이 저장소를 GitHub에 푸시합니다.
2. [Vercel](https://vercel.com)에 로그인 → **Add New… → Project** → 이 저장소 **Import**.
3. **Environment Variables**에 다음을 추가합니다.
   - Name: `GROQ_API_KEY`
   - Value: *(Groq 콘솔에서 발급한 키)*
4. **Deploy** 클릭. 배포가 끝나면 제공되는 URL에서 챗봇이 동작합니다.

> 환경변수를 나중에 추가/변경했다면 **Redeploy** 해야 적용됩니다.

---

## 💻 로컬에서 실행하기

Vercel CLI로 서버리스 함수까지 함께 실행할 수 있습니다.

```bash
# 1) Vercel CLI 설치 (최초 1회)
npm i -g vercel

# 2) 환경변수 설정 (둘 중 하나)
#    - 프로젝트 루트에 .env 파일 생성 후 GROQ_API_KEY=... 작성
#    - 또는 셸에서: export GROQ_API_KEY=...

# 3) 로컬 개발 서버 실행 (http://localhost:3000)
vercel dev
```

> 단순히 `public/index.html`만 브라우저로 열면 UI는 보이지만 `/api/chat` 함수가
> 없어 답변이 오지 않습니다. 답변까지 테스트하려면 `vercel dev`로 실행하세요.

---

## ⚙️ 모델 변경

기본 모델은 `llama-3.3-70b-versatile`입니다. `api/chat.js` 상단의 `MODEL` 상수만 바꾸면 됩니다.

```js
const MODEL = "llama-3.3-70b-versatile"; // 예: "llama-3.1-8b-instant", "gemma2-9b-it"
```

> 사용 가능한 모델 목록은 https://console.groq.com/docs/models 에서 확인할 수 있습니다.

---

## 🧩 다른 웹페이지(내 사이트)에 위젯 붙이기

배포가 끝나면, **아무 HTML 페이지의 `</body>` 바로 앞에 아래 한 줄만 추가**하면
우측 하단에 채팅 버블이 나타납니다.

```html
<script src="https://<당신의-vercel-주소>/widget.js" defer></script>
```

예: 배포 주소가 `https://salesforcechatbot.vercel.app` 라면

```html
<script src="https://salesforcechatbot.vercel.app/widget.js" defer></script>
```

- `widget.js`는 자신이 로드된 주소를 보고 `/api/chat`을 **자동으로 같은 Vercel
  도메인으로 호출**하므로, 다른 도메인 사이트에 붙여도 그대로 동작합니다.
- API(`api/chat.js`)에 **CORS가 허용**되어 있어 외부 도메인에서의 호출도 막히지 않습니다.
- 클래스/ID에 `sfgw-` 접두사를 써서 기존 사이트 디자인과 충돌하지 않습니다.

> 💡 같은 Vercel 프로젝트의 `index.html`도 동일하게 `widget.js`를 불러옵니다.
> 즉, 배포 URL을 그냥 열면 데모 페이지에서 위젯을 바로 확인할 수 있습니다.
