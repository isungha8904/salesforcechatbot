...
AI 챗봇입니다. Google **Gemini API**로 답변을 생성하고, HTML 프론트엔드로 동작하며,
**Vercel**로 배포합니다.
...
├── api/
│   └── chat.js         # Vercel 서버리스 함수: Gemini API 프록시
├── vercel.json         # 라우팅 설정
...
Google **AI Studio**에서 발급하는 **무료 Gemini API 키** 1개가 필요합니다.

1. https://aistudio.google.com/app/apikey 접속
2. **Create API key** 클릭 → 키 복사
3. 이 키를 코드에 넣지 말고, 아래 Vercel 환경변수(`GEMINI_API_KEY`)로 등록합니다.
...
# 2) 환경변수 설정 (둘 중 하나)
#    - 프로젝트 루트에 .env 파일 생성 후 GEMINI_API_KEY=... 작성
#    - 또는 셸에서: export GEMINI_API_KEY=...
...
기본 모델은 `gemini-2.0-flash`입니다. `api/chat.js` 상단의 `MODEL` 상수만 바꾸면 됩니다.

```js
const MODEL = "gemini-2.0-flash"; // 예: "gemini-1.5-flash", "gemini-1.5-pro"
```
